...
router.post('/shakka', middleware.requiresUser, function(req, res) {
    ...
    if(type == 'event'){
        ...
        Post.addToShakka( req , function(err, post) {
...
router.delete('/shakka', middleware.requiresUser, function(req, res) {
    var type = req.body.type;
    if(type == 'event'){
        ...
        Post.removeShakka( req , function(err, post) {
                res.send({"status":"success", "post":post});
        });
    }else{
        res.send('Missing type parameter');
